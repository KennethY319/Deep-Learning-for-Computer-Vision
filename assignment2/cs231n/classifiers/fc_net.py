...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        regularize=0.5 * self.reg * (np.sum(W1 * W1) + np.sum(W2 * W2))
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # layer, etc.                                                              #
        ############################################################################
        hidden_layer, cache1, cache2, cache3, batchnorm, out = {}, {}, {}, {}, {}, {}
        cache4={}
        out[0] = X
        ...
        for i in range(self.num_layers - 1):
            ...
            if self.use_batchnorm:
                ...
                hidden_layer[i], cache1[i] = affine_forward(out[i], W, b)
                batchnorm[i], cache2[i] = batchnorm_forward(hidden_layer[i], gamma, beta, self.bn_params[i])
                out[i + 1], cache3[i] = relu_forward(batchnorm[i])
                if self.use_dropout:
        ...
        ddropout={}
        k = self.num_layers - 1
        dout[k], grads['W' + str(k + 1)], grads['b' + str(k + 1)] = affine_backward(dscores, cache)
        for i in range(k):
            if self.use_dropout:
                ddropout[k - 1 - i] = dropout_backward(dout[k - i], cache4[k - 1 - i])
                dout[k - i] = ddropout[k - 1 - i]
            if self.use_batchnorm:
                dbn[k - 1 - i] = relu_backward(dout[k - i], cache3[k - 1 - i])
                dh[k - 1 - i], grads['gamma' + str(k - i)], grads['beta' + str(k - i)] = \
                    batchnorm_backward(dbn[k - 1 - i], cache2[k - 1 - i])
                dout[k - 1 - i], grads['W' + str(k - i)], grads['b' + str(k - i)] = \
                    affine_backward(dh[k - 1 - i],cache1[k - 1 - i])
            else:
                dout[k - 1 - i], grads['W' + str(k - i)], grads['b' + str(k - i)] = \
                    affine_relu_backward(dout[k - i],cache3[k-1-i])
